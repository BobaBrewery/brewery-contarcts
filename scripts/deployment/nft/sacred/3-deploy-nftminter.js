...
async function main() {
    ...
    // nft holder : deployer
    const minter = await Minter.deploy(admin_contract.address, nft_address, usdt_address, deployer.address);
    ...
    await nft.connect(deployer).setApprovalForAll(minter.address, true)
    console.log("is minter approved? ", await nft.isApprovedForAll(deployer.address, minter.address))
