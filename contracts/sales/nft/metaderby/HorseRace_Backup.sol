...
contract HorseRace_Backup is ReentrancyGuard, Pausable {
    ...
    function whitelist(bytes32[] calldata merkleProof, uint256 _horseId)
        external
        nonReentrant
    {
        require(!whitelistInfo[msg.sender], "Address has already claimed");
        require(
            MerkleProof.verify(
                merkleProof,
                merkleRoot,
        ...
        whitelistInfo[msg.sender] = true;
        betInfos[_horseId][msg.sender] = betInfos[_horseId][msg.sender].add(
            breAirdrop
        );
        totalStakeById[_horseId] = totalStakeById[_horseId].add(breAirdrop);
