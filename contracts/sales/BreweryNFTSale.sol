...
contract BreweryNFTSale is ReentrancyGuard {
    ...
    function setVestingParams(
        ...
        require(
            ...
            "vestingPerPortion params not provided"
        );
        require(
    ...
    function setRegistrationTime(
        ...
        require(
            ...
            "registrationTimeStarts should be 0"
        );
        require(
            _registrationTimeStarts >= block.timestamp &&
            _registrationTimeEnds > _registrationTimeStarts,
            "registration time was wrongly set"
        );
        require(
            _registrationTimeEnds < sale.saleEnd,
            "_registrationTimeEnds < sale.saleEnd is not allowed"
        );

        if (sale.saleStart > 0) {
            require(
                _registrationTimeEnds < sale.saleStart,
                "registrationTimeEnds >= sale.saleStart is not allowed"
            );
        }
    ...
    function setSaleStart(uint256 starTime) external onlyAdmin {
        require(sale.isCreated, "sale is not created.");
        require(sale.saleStart == 0, "setSaleStart: starTime is set already.");
        require(
            starTime > registration.registrationTimeEnds,
            "start time should greater than registrationTimeEnds."
        );
        require(
            starTime < sale.saleEnd,
            "start time should less than saleEnd time"
        );
        require(
    ...
    function registerForSale(bytes memory signature, uint256 pid) external {
        ...
        require(!isRegistered[msg.sender], "User can not register twice.");
    ...
    function postponeSale(uint256 timeToShift) external onlyAdmin {
        require(block.timestamp < sale.saleStart, "sale already started.");
    ...
    function extendRegistrationPeriod(uint256 timeToAdd) external onlyAdmin {
        require(
            registration.registrationTimeEnds.add(timeToAdd) < sale.saleStart,
    ...
    payable
    {
        ...
        require(
            sale.totalTokensSold <= sale.amountOfTokensToSell,
            "token is sold out!"
        );
    ...
    nonReentrant
    {
        ...
        for (uint256 i = 0; i < portionIds.length; i++) {
            ...
            require(
                portionId < vestingPercentPerPortion.length,
                "portion id is larger than portion length"
            );
